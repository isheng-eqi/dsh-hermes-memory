...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Panel() {
      ...
      const refresh = () => {
        loadAll().then((d) => { setData(d); setNotice(null) }).catch(() => setNotice({ ok: false, text: '载入失败' }))
      }
      React.useEffect(() => {
        if (v) refresh()
      }, [v])
      ...
      const filter = (list) => (q ? (list || []).filter((t) => String(t).toLowerCase().includes(q)) : (list || []))
      ...
      const memList = filter(data ? data.memory : null)
      const userList = filter(data ? data.user : null)
    ...
    function apply(ctx) {
      ...
      if (!document.getElementById(cssId)) {
        ...
        st.id = cssId
        st.textContent = `
          .hm-row:hover .hm-action { opacity: 1 !important; }
